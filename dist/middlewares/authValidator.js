...
function () {
  ...
  _createClass(AuthValidation, null, [{
    ...
    key: "EmployeePasswordValidator",
    value: function EmployeePasswordValidator(req, res, next) {
      var schema = _joi["default"].object().keys({
        password: _joi["default"].string().min(6).label('password').required()
      });
      ...
      if (valid.error != null) {
        return res.status(400).json({
          status: 400,
          error: "".concat(valid.error.details[0].message)
        });
      }
